...
import loginbg from "../image/pic1.png";
...
useEffect(() => {
  ...
  return (
		<div className="authincation d-flex flex-column flex-lg-row flex-column-fluid position-relative">

		{camera && <div className='camera'>

			<div className='vid'>
				<video ref={videoRef}></video>
				<button onClick={takePhoto}>{t("take")}</button>
			</div>

			<div className={'result ' + (hasPhoto ? 'hasPhoto' : '')}>
				<canvas ref={photoRef}></canvas>
				{pen && <button onClick={closePhoto}>{t("close")}</button>}
			</div>
		</div>}
           ...
           <div className='ope'>
			  <button className='bt'  onClick={() => setCamera(true)}>{t("open")}</button>
			  {camera && <button className='bt1'  onClick={() => setCamera(false)}>{t("close")}</button>}
		   </div>
			...
			<div className="login-aside text-center  d-flex flex-column flex-row-auto">
				<div className="d-flex flex-column-auto flex-column pt-lg-40 pt-15">
					...
					<p>User Experience & Interface Design <br />Interface Design</p>
				</div>
			...
			<div className='crud'>
				...
				<div className='form'>
					<AddContact addContactHandler={addContactHandler} t={t} 
				/>
				</div>
				<div className='tab'>
					<ContactList contacts={contacts} getContactId={removeContactHandler} t={t}  />
				</div>
			</div>
			<Routes>
      <Route path="edit" element={<EditContact addContactHandler={addContactHandler} />}></Route>
    </Routes>
		</div>	
